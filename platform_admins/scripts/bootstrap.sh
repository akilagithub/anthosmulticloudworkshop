...
grep -q "export ASM_VERSION.*" ${SCRIPT_DIR}/../../../vars.sh || echo -e "export ASM_VERSION=1.13.4-asm.4" >> ${SCRIPT_DIR}/../../../vars.sh
...
title_no_wait "Verifying that you are logged in with the correct user. The user should be ${GCLOUD_USER}."
...
if [[ ! $API_ENABLED ]]; then
  ...
  # anthosconfigmanagement temporary until gcloud issue resolved for hub enable
...
if [[ ! $TF_CLOUDBUILD_SA ]]; then
  ...
  title_no_wait "Giving Cloudbuild SA service usage admin role"
  print_and_execute "gcloud projects add-iam-policy-binding ${GOOGLE_PROJECT} \
  --member serviceAccount:${TF_CLOUDBUILD_SA} \
  --role roles/serviceusage.serviceUsageAdmin"

  title_no_wait "Giving Cloudbuild SA project IAM admin role"
  print_and_execute "gcloud projects add-iam-policy-binding ${GOOGLE_PROJECT} \
  --member serviceAccount:${TF_CLOUDBUILD_SA} \
...
title_no_wait "Ensure AWSServiceRoleForElasticLoadBalancing AWS IAM role exists"
if aws iam get-role --role-name=AWSServiceRoleForElasticLoadBalancing &> /dev/null;  then
  title_no_wait "AWSServiceRoleForElasticLoadBalancing role already exists"
else
  title_no_wait "Creating AWSServiceRoleForElasticLoadBalancing role"
...
if [ $? -eq 0 ]; then
    echo
    echo "------------------------------------------------------------------------------------------"
    echo "| Build progress can be monitored at https://console.cloud.google.com/cloud-build/builds |"
    echo "------------------------------------------------------------------------------------------"
    echo
fi
