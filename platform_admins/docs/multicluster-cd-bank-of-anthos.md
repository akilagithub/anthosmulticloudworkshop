...
- [📽️ Video Walk Through I: Prerequisites and starting pipeline](https://threadit.app/thread/e7xbr09np7402s136il4?utm_medium=referral-link)
- [📽️ Video Walk Through II: Accessing the application and inspecting CockroachDB](https://threadit.app/thread/gnynsmqlti6kv3nek4si?utm_medium=referral-link)
- [📽️ Video Walk Through III: Inspecting CI/CD pipeline and metrics](https://threadit.app/thread/bd3czy6h8x16gl6355bu?utm_medium=referral-link)
- [📽️ Video Walk Through IV: Migrating workloads using CI/CD pipelines](https://threadit.app/thread/h9tox4iwnpcjkrxofcn5?utm_medium=referral-link)

## Prerequisites

1. Initialize the `shared-cd` repository. Follow the steps [here](/platform_admins/docs/init-shared-cd-repo.md). `shared-cd` repository contains the CI/CD jobs/stages required to deploy Bank of Anthos to the platform.
1. Initialize the `config` repository. Follow the steps [here](/platform_admins/docs/init-config-repo.md). `config` repository ensures that the Bank of Anthos namespaces are created in all clusters in the `prod` environment.
