...
gcloud beta container hub ingress enable \
  --config-membership=${CLUSTER_NAME} --project ${PROJECT_ID} || true

# ingress enabling can timeout after 2 minutes, but still be OK, validate accordingly
for i in {1..30}; do
  RESULT=$(gcloud beta container hub ingress describe --project ${PROJECT_ID} --format="value(state.state.code)")
  if [ ${RESULT} = "OK" ]; then
    break;
  fi
